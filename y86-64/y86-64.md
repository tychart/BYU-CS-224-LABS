...
# Where to start?
 ...
 5. Implement `HALT` by adding code to `fetchStage` and `pcUpdateStage` (make sure that the PC is updated to the address of the next instruction and the status is set the HLT)
